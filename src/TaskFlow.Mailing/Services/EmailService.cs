...
namespace TaskFlow.Mailing.Services
{
    public class EmailService : IEmailService
    {
        ...
        {
            var confirmationLink = $"http://localhost:4200/auth/confirm-email?email={email}&token={Uri.EscapeDataString(token)}";
        ...
        {
            var resetLink = $"http://localhost:4200/auth/reset-password?email={email}&token={Uri.EscapeDataString(token)}";
            var emailDto = new EmailDto
            {
                To = email,
